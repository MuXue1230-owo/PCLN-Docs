...
`PclUiService` 是不暴露 Avalonia 私有类型的首选完整页面接口。宿主负责以 PCL N 原版控件、主题、字号和窗口边界渲染 `PclUiCard`、`PclUiTextBox`、`PclUiButton` 等组件。所有文字使用 `PclLocalizedString`：

```csharp
PclUiService ui = context.Services.Require<PclUiService>();
PclUiPageRegistration page = ui.RegisterDynamicPage(new PclUiPage
{
    OperationId = "dev.example.toolbox.page-registration",
    Route = "dev.example.toolbox.page",
    Title = new PclLocalizedString("toolbox.title", "实例工具箱"),
    Icon = "lucide/wrench",
...
ui.EventRaised += (_, e) =>
{
    if (e is { ElementId: "refresh", Kind: PclUiEventKind.Click })
        _ = page.UpdateContentAsync(BuildContent());
};
```

`RegisterDynamicPage` 保持导航元数据稳定，只替换页面内容；它可从后台线程调用，Host 会在 UI dispatcher 中重绘当前打开的所有页面实例。Slot 内容需要动态刷新时使用 `InjectDynamic`。静态页面仍可使用 `RegisterPage` / `Inject`。

带参数的多语言文本使用 `.Format(...)`，不要先在插件中固定为某一种语言：

```csharp
new PclLocalizedString("status.address", "联机地址：{0}").Format(address)
```
